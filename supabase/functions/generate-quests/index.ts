...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const filledPrompt = fillPromptTemplate(prompt.prompt, {
      ...
      recent_quests: JSON.stringify(context.recentQuests.map(q => q.quest_id)),
    })
    ...
    const generatedQuests = parseAIResponse<{ quests: unknown[] }>(aiResponse)

    // 6. Insert generated quests into quests table
    const questsToInsert = (generatedQuests.quests ?? []).map((q: Record<string, unknown>) => ({
