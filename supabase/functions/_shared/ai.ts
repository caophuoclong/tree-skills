 ...
 */
function parseResponse(provider: AIProvider, data: Record<string, unknown>): string {
  switch (provider.name) {
    case 'openai':
    ...
    case 'anthropic':
      return (data as { content?: Array<{ text?: string }> })
        ?.content?.[0]?.text ?? '{}'
...
export async function generateWithAI(
  ...
  const apiKey = provider.api_key_env ? Deno.env.get(provider.api_key_env) ?? '' : ''
  ...
  if (!response.ok) {
    ...
    throw new Error(`AI API error (${provider.name}): ${response.status} - ${error}`)
  }
