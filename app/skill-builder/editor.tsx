...
export default function EditorScreen() {
  ...
  return (
    ...
    >
      <View
        style={[styles.editorHeader, { borderBottomColor: colors.glassBorder }]}
      >
        ...
        <View style={{ flex: 1, alignItems: "center" }}>
          <Text
            style={[styles.editorHeaderTitle, { color: colors.textPrimary }]}
          >
            Chỉnh sửa lộ trình
          </Text>
          <Text
            style={[styles.editorHeaderSub, { color: colors.textMuted }]}
            numberOfLines={1}
          >
            {currentDraft.goal}
          </Text>
        ...
        <TouchableOpacity onPress={handleConfirm} disabled={totalSkills === 0}>
          <Text
            style={[
              styles.headerAction,
              {
                color: totalSkills > 0 ? colors.brandPrimary : colors.textMuted,
                fontWeight: "800",
              },
            ]}
          >
      ...
      <View style={[styles.statsStrip, { backgroundColor: colors.bgSurface }]}>
        ...
        <View
          style={[styles.statDivider, { backgroundColor: colors.glassBorder }]}
        />
        <View style={styles.statItem}>
          <Text style={[styles.statNum, { color: colors.textPrimary }]}>
            {totalSkills}
        ...
        >
          {(Object.keys(BRANCH_CONFIG) as Branch[]).map((b) => {
            const count = currentDraft.clusters.filter(
              (c) => c.branch === b,
            ).length;
            ...
            return (
              ...
              >
                ...
                <Text style={[styles.miniChipCount, { color: cfg.color }]}>
                  {count}
                </Text>
              </View>
            );
      ...
      >
        ...
        <Text style={[styles.hintText, { color: colors.textMuted }]}>
          Tap tên để sửa · Tap{" "}
          <Text style={{ color: colors.brandPrimary }}>☰</Text> để mở/sửa nhiệm
          vụ · Tap badge khó/thời gian để đổi
        </Text>
      </View>
      ...
      >
        {currentDraft.clusters.map((cluster) => (
          <SkillBuilderClusterCard
            ...
            onRemove={() =>
              Alert.alert(
                "Xoá nhóm này?",
                `"${cluster.title}" và tất cả kỹ năng bên trong sẽ bị xoá.`,
                [
                  { text: "Huỷ", style: "cancel" },
                  {
                    text: "Xoá",
                    style: "destructive",
                    onPress: () => removeCluster(cluster.id),
                  },
                ],
              )
            }
        ...
        >
          <Ionicons
            ...
            color={colors.brandPrimary}
          />
          <Text style={[styles.addClusterText, { color: colors.brandPrimary }]}>
            Thêm nhóm kỹ năng
          </Text>
        </TouchableOpacity>
